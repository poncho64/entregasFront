...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["foto_paciente"]) && $_FILES["foto_paciente"]["error"] == 0) {
        ...
        if (move_uploaded_file($_FILES["foto_paciente"]["tmp_name"], $ruta_completa)) {
            $sql = "INSERT INTO pacientes (doc_paciente, nombre_paciente, apellido_paciente, celular_paciente, correo_paciente, contrasena_paciente, foto_paciente)
                    VALUES (?, ?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssssss", $doc_paciente, $nombre_paciente, $apellido_paciente, $celular_paciente, $correo_paciente, $contrasena_paciente, $ruta_completa);
